...
export const UserManagementCard = ({ user }: UserManagementCardProps) => {
  ...
  const isUserActive = user.accountStatus === 'ACTIVE';

  const handleUserAction = async (action: 'block' | 'unblock') => {
    try {
      const data = await updateStatus.mutateAsync({ userId: user._id, action });
      if (data.success) {
      ...
      <Card>
        <CardHeader className="flex flex-row items-center gap-3 pb-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-muted-foreground">Manage user account access</p>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 pt-0">
          ...
          <div className="bg-muted/40 rounded-lg p-3.5 border border-border">
            <div className="flex flex-col gap-3">
              <div>
                <h5 className="font-medium text-sm text-foreground">{user.name || 'User'}</h5>
            ...
            <p className="text-sm text-muted-foreground mt-3">
              {isUserActive
                ? 'Blocking will suspend this user\'s account and prevent them from making calls on the platform.'
                : 'Unblocking will restore this user\'s account access and allow them to make calls again.'
              }
            </p>
          </div>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm text-amber-800 dark:text-amber-200 leading-relaxed">
                  Account blocking actions are immediate and will affect the user's ability to access the platform and make calls.
          ...
          <div className="space-y-2.5">
            ...
            <div className="grid gap-2.5">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <span className={`text-sm font-medium ${user.totalComplaints > 0 ? 'text-red-600' : 'text-green-600'}`}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {isUserActive ? (
                <>
                  Are you sure you want to block <span className="font-medium">{user.name || 'this user'}</span>?
                  <p className="mt-2">This will prevent them from making calls and accessing the platform.</p>
                </>
              ) : (
                <>
                  Are you sure you want to unblock <span className="font-medium">{user.name || 'this user'}</span>?
                  <p className="mt-2">This will restore their access to make calls on the platform.</p>
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
