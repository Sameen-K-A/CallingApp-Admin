...
import type { IUserComplaint } from "@/types/general";
...
export const UserComplaintsCard = ({ complaints, totalComplaints }: UserComplaintsCardProps) => {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center gap-3 pb-4">
        ...
        <div className="min-w-0 flex-1">
          <CardTitle className="text-lg">Complaints Against User</CardTitle>
          <p className="text-sm text-muted-foreground">
            {totalComplaints > 0
              ? `Showing ${Math.min(5, complaints.length)} of ${totalComplaints} total complaints`
              : 'No complaints filed against this user'
            }
          </p>
        </div>
      </CardHeader>
        ...
        {complaints.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 text-center border rounded-lg bg-muted/20">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              This user has a clean record with no complaints filed against them.
            </p>
          </div>
        )}
      </CardContent>
